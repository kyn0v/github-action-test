# refer to: https://github.com/zombie12138/nuaa_check_action/blob/main/check.py
# encoding=utf-8
import traceback
import re
import json
...
attempts_max = 1    # 最大尝试次数
delay = 2   # 访问页面前的延迟，防止过快访问网站被封IP
...
def login(stu_number, password):
    ...
    for _ in range(attempts_max):
        try:
            time.sleep(delay)
            response = requests.get('https://m.nuaa.edu.cn/uc/wap/login', cookies=cookies)
            print('get login page:', response.status_code)
            # cookies = response.headers['Set-Cookie']
            # cookies = re.search(r'eai-sess=([a-zA-Z0-9]+)', cookies).group(0)
            cookies = dict(response.cookies)
            time.sleep(delay)
            response = requests.get('https://m.nuaa.edu.cn/uc/wap/login/check', cookies=cookies, data='username={}&password={}'.format(stu_number, password))
            print('login checkin-page:', response.status_code)
    ...
    return {}, '登陆结果：login failed,请检查账号密码\n'
...
def check(cookies, geo_api_info, id, uid):
    ...
    for _ in range(attempts_max):
        try:
            ...
            print('signin state code:', response.status_code)
            # print('signin return:', response.text)
            response.encoding = 'utf-8'
...
def send_result(tg_to, tg_token):
    updater = Updater(token=tg_token, use_context=True)
    updater.bot.send_message(chat_id=tg_to, text='打卡成功！')
